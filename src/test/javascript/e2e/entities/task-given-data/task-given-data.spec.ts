...
describe('TaskGivenData e2e test', () => {
    ...
    it('should create and save TaskGivenData', async () => {
        ...
        expect(await taskGivenDataUpdatePage.getContentInput()).to.eq('content');
        await taskGivenDataUpdatePage.save();
        expect(await taskGivenDataUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await taskGivenDataComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
